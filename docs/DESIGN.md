...
- ~~`cli.py` does not yet expose several engine/pairing capabilities~~
  **Closed.** `cli.py` now has full parity with the GUI's pairing/
  contacts capabilities: `whoami`/`quickshare`/`pasteconnect` (the
  same one-click pairing flow), `contacts`/`connectto`/`alias`/
  `forget`, `cancel` (calls `cancel_transfer`), and `snapshot` (prints
  `get_state_snapshot()`). Also fixed the same restart gap the GUI's
  `Bridge.__init__` already fixed: `cli.py` now calls
  `pairing.load_contacts_into_engine()` at startup, so previously-
  paired contacts (via `quickshare`/`pasteconnect` in an earlier run)
  aren't forgotten every time the CLI restarts. `trust`/`connect`
  remain as the manual/ephemeral flow (useful for testing without a
  full contact record) - not extended to include `rename`, matching
  the GUI's own decision to keep only the cosmetic `alias`, not a true
  rename, given how narrow rename's actual value turned out to be.
  Verified via a real, live multi-process test: a genuine invite
  generated by one running `cli.py` instance, pasted and connected by
  a second, contacts/alias/connectto/forget all confirmed working
  against the real saved contact.
